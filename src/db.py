...
logging.basicConfig(
    filename=str(BASE_DIR /"data_store"/"audit.log"),
    level=logging.INFO,
    ...
    *,
    ...
    with SessionLocal() as db:
        ...
        try:
            db.commit()
            logging.info(
                f"Visit saved | case_id={case_id} | date={visit_date} | eye={eye} | mode={mode} | visit_id={visit_id}"
            )
        except IntegrityError:
            db.rollback()
            raise ValueError(
                f"This visit already exists for Case {case_id} on {visit_date} ({eye}, {mode})."
            )
...
def delete_case(case_id: str):
    ...
    with SessionLocal() as db:
        db.execute(sql_delete(Visit).where(Visit.case_id == case_id))
        db.execute(sql_delete(Case).where(Case.case_id == case_id))
        db.commit()
        logging.info(f"Case deleted | case_id={case_id}")