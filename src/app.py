...
def inject_css():
    ...
    st.warning("This is a research prototype only. Not for clinical use. Case IDs must be anonymised. No patient identifiable information should be entered.")
...
#Display metrics in streamlit st.metric blocks 
    ...
    if st.button("Save this visit", disabled=not save_enabled):
        try:
            ...
            "This exact visit/image has already been saved for this case."
...
### Data and privacy
- Case IDs must be anonymised, do not enter patient names or NHS numbers
- This tool is a research prototype and is not approved for clinical use
"""
    )
...
with tab_workflow:
    with st.sidebar:
        ...
        st.divider()
        if st.sidebar.button("Delete this case", type="secondary"):
            case_id = st.session_state.get("case_id", "").strip()
            if case_id:
                from db import delete_case
                delete_case(case_id)
                st.sidebar.success(f"Case {case_id} deleted.")
            else:
    ...
    with left:
        ...
        with st.expander("**Privacy notice**", expanded=False):
            st.markdown("""
        **What this tool is**
        research prototype, not approved for clinical use and must not be used to inform clinical decisions.

        **What data is collected**
        - Case reference ID, must be an anonymised code only. Do not enter patient names, NHS numbers or any identifiable information.
        - Visit date, eye, computed measurements and auto-generated report, stored locally on this device only.

        **What is NOT collected**
        - The uploaded image is never stored. Only a short hash fingerprint is kept to prevent duplicate entries.
        - No patient identifiable information is collected.

        **Third party services**
        The Groq API is used to generate the clinical note. A summary of computed measurements only (no images, no patient identifiers) is sent to Groq's servers. No other data leaves this device.

        **Data storage and deletion**
        All data is stored in a local SQLite database at `data_store/app.db`. To delete all data, delete that file. To delete a specific case, use the Delete button in the sidebar.

        **GDPR**
        This is an academic research prototype. A full DPIA would be required before any real clinical deployment under UK GDPR and the Data Protection Act 2018.
        """)
        st.markdown(
            "<div style='font-size:0.75rem; opacity:0.45; text-align:center; padding:4px 0;'>Research prototype — not for clinical use | Data stored locally only</div>",
            unsafe_allow_html=True,
        )